...
#ifndef CAV_SRC_SKA_SORT_HPP
#define CAV_SRC_SKA_SORT_HPP
...
struct SizedRadixSorter<2> {
    ...
    static bool sort(It begin, It end, OutIt buffer_begin, ExtractKey&& extract_key) {
        ...
        if (num_elements <= (1ll << 32))
            return sort_inline<uint32_t>(begin,
                                         end,
                                         buffer_begin,
                                         buffer_begin + num_elements,
                                         extract_key);
        else
            return sort_inline<uint64_t>(begin,
                                         end,
                                         buffer_begin,
                                         buffer_begin + num_elements,
                                         extract_key);
    ...
    template <typename count_type, typename It, typename OutIt, typename ExtractKey>
    static bool sort_inline(It           begin,
                            It           end,
                            OutIt        out_begin,
                            OutIt        out_end,
...
template <typename K, typename V>
struct RadixSorter<std::pair<K, V> const&> {
    template <typename It, typename OutIt, typename ExtractKey>
    static bool sort(It begin, It end, OutIt buffer_begin, ExtractKey&& extract_key) {
        bool first_result = RadixSorter<V>::sort(
            ...
            buffer_begin,
            [&](auto&& o) -> V const& { return extract_key(o).second; });
        auto extract_first = [&](auto&& o) -> K const& { return extract_key(o).first; };
...
template <size_t I, size_t S, typename Tuple>
struct TupleRadixSorter<I, S, Tuple const&> {
    using NextSorter = TupleRadixSorter<I + 1, S, Tuple const&>;
    using ThisSorter = RadixSorter<typename std::tuple_element<I, Tuple>::type>;
...
template <size_t I, typename Tuple>
struct TupleRadixSorter<I, I, Tuple const&> {
    template <typename It, typename OutIt, typename ExtractKey>
    static bool sort(It, It, OutIt, OutIt, ExtractKey&&) {
        return false;
    }
...
template <typename... Args>
struct RadixSorter<std::tuple<Args...> const&> {
    using SorterImpl = TupleRadixSorter<0, sizeof...(Args), std::tuple<Args...> const&>;
...
template <typename T>
struct RadixSorter<T const> : RadixSorter<T> {};

template <typename T>
struct RadixSorter<T&> : RadixSorter<T const&> {};
...
struct PairSecondSubKey : Current {
    static decltype(auto) sub_key(std::pair<F, S> const& value, void* sort_data) {
...
struct TupleSubKey : Current {
    ...
    static decltype(auto) sub_key(Tuple const& value, void* sort_data) {
...
struct ListElementSubKey : SubKey<typename std::decay<decltype(std::declval<T>()[0])>::type> {
    ...
    static decltype(auto) sub_key(U&& value, void* sort_data) {
        ...
        T const&          list = CurrentSubKey::sub_key(value, list_sort_data->next_sort_data);
...
struct ListSubKey {
    ...
    static T const& sub_key(T const& value, void*) {
        return value;
    }
};
...
inline bool StdSortIfLessThanThreshold(It             begin,
                                       It             end,
...
struct UnsignedInplaceSorter {
    ...
    template <typename T>
    static inline uint8_t current_byte(T&& elem, void* sort_data) {
    ...
    recurse:
        if (Offset + 1 != NumBytes || next_sort) {
            ...
            for (uint8_t *it = remaining_partitions, *end = remaining_partitions + num_partitions;
                ...
                if (!StdSortIfLessThanThreshold<StdSortThreshold>(partition_begin,
                                                                  partition_end,
                                                                  num_elements,
                                                                  extract_key)) {
    ...
    static void ska_byte_sort(It          begin,
        ...
        if (Offset + 1 != NumBytes || next_sort) {
            for (uint8_t* it = remaining_partitions + num_partitions; it != remaining_partitions;
                 --it) {
...
struct UnsignedInplaceSorter<StdSortThreshold,
                             ...
                             NumBytes> {
    template <typename It, typename ExtractKey>
    static inline void sort(It             begin,
                            It             end,
                            std::ptrdiff_t num_elements,
                            ExtractKey&    extract_key,
                            void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey&, void*),
...
size_t CommonPrefix(It           begin,
                    It           end,
                    size_t       start_index,
                    ExtractKey&& extract_key,
                    ElementKey&& element_key) {
    auto const& largest_match_list = extract_key(*begin);
    size_t      largest_match      = largest_match_list.size();
    if (largest_match == start_index)
        return start_index;
    for (++begin; begin != end; ++begin) {
        auto const& current_list = extract_key(*begin);
...
struct ListInplaceSorter {
    ...
    static void sort(It                            begin,
        ...
        sort_data->current_index = current_index = CommonPrefix(begin,
                                                                end,
                                                                current_index,
                                                                current_key,
                                                                element_key);
        ...
        if (sort_data->next_sort &&
            !StdSortIfLessThanThreshold<StdSortThreshold>(begin,
                                                          end_of_shorter_ones,
                                                          num_shorter_ones,
                                                          extract_key)) {
            sort_data->next_sort(begin,
                                 end_of_shorter_ones,
                                 num_shorter_ones,
                                 extract_key,
                                 next_sort_data);
        ...
        if (!StdSortIfLessThanThreshold<StdSortThreshold>(end_of_shorter_ones,
                                                          end,
                                                          num_elements,
                                                          extract_key)) {
            void (*sort_next_element)(It, It, std::ptrdiff_t, ExtractKey&, void*) = static_cast<
                void (*)(It, It, std::ptrdiff_t, ExtractKey&, void*)>(&sort_from_recursion);
    ...
    template <typename It, typename ExtractKey>
    static void sort_from_recursion(It begin,
                                    It end,
                                    std::ptrdiff_t,
                                    ExtractKey& extract_key,
                                    void*       next_sort_data) {
        ListSortData<It, ExtractKey> offset = *static_cast<ListSortData<It, ExtractKey>*>(
            next_sort_data);
...
template <std::ptrdiff_t StdSortThreshold,
          ...
          typename SubKeyType>
struct InplaceSorter
    : FallbackInplaceSorter<StdSortThreshold,
                            AmericanFlagSortThreshold,
                            CurrentSubKey,
                            SubKeyType> {};
...
struct SortStarter {
    ...
    static void sort(It             begin,
        ...
        void (*next_sort)(It, It, std::ptrdiff_t, ExtractKey&, void*) = static_cast<
            void (*)(It, It, std::ptrdiff_t, ExtractKey&, void*)>(
...
void inplace_radix_sort(It begin, It end, ExtractKey& extract_key) {
    ...
    SortStarter<StdSortThreshold, AmericanFlagSortThreshold, SubKey>::sort(begin,
                                                                           end,
                                                                           end - begin,
                                                                           extract_key);
}
...
bool ska_sort_copy(It begin, It end, OutIt buffer_begin, ExtractKey&& key) {
    ...
    if (num_elements < 128 ||
        ...
        return detail::RadixSorter<
            typename std::result_of<ExtractKey(decltype(*begin))>::type>::sort(begin,
                                                                               end,
                                                                               buffer_begin,
                                                                               key);
}
